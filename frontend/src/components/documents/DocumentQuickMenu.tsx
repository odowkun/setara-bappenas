...
export function DocumentQuickMenu({
  ...
  // Fetch dynamic running text from API
  useEffect(() => {
    if (!isTickerMode) return;
    runningTextService
      .getPublicItems()
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          const mapped: TickerItem[] = data.map((item) => ({
            id: String(item.id),
            title: item.content,
      ...
      .catch((err) => {
        console.warn("[DocumentQuickMenu] Gagal memuat running text:", err);
      });
  }, [isTickerMode]);
        ...
        createPortal(
          <div className="fixed bottom-3 sm:bottom-6 left-0 right-0 max-w-7xl mx-auto px-3 sm:px-6 z-[70] pointer-events-none">
            <nav
              ...
              className={`pointer-events-auto w-full rounded-2xl sm:rounded-full border border-slate-200/90 bg-white/95 p-2 sm:p-2 shadow-2xl shadow-blue-950/20 backdrop-blur-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2 overflow-hidden relative sm:min-h-[62px] ${className}`}
            >
              {/* Top Line (Mobile) / Left Side (Desktop): Running Text (Marquee Ticker) */}
              <div
                className={`w-full sm:flex-1 flex items-center gap-2 sm:gap-3 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] min-w-0 ${
                  categoriesRevealed ? "sm:mr-3" : "w-full"
                }`}
              >
                {/* Sleek Live Indicator without "INFO HALUT" text */}
                <div
                  className="shrink-0 flex items-center justify-center w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-blue-600/10 border border-blue-500/25 text-blue-600 shadow-xs select-none"
                  title="Warta Terkini BAPPEDA"
                >
                  <span className="relative flex h-2 sm:h-2.5 w-2 sm:w-2.5">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-500 opacity-75" />
                    <span className="relative inline-flex rounded-full h-2 sm:h-2.5 w-2 sm:w-2.5 bg-blue-600" />
                  </span>
                </div>

                {/* Continuous Marquee Ticker Track */}
                <div className="relative flex-1 overflow-hidden mask-fade-edges py-0.5 sm:py-1">
                  <div className="animate-bappeda-marquee flex items-center gap-6 sm:gap-8 whitespace-nowrap will-change-transform">
                    {doubledTicker.map((item, idx) => (
                      <Link
                        key={`${item.id}-${idx}`}
                        href={item.href}
                        className="group inline-flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-sm font-semibold text-slate-700 hover:text-blue-700 transition-colors"
                      >
                        <span className="text-[9px] sm:text-[10px] font-black uppercase px-1.5 sm:px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 border border-blue-200/60 group-hover:bg-blue-600 group-hover:text-white transition">
                          {item.tag}
                        </span>
                        <span className="hover:underline line-clamp-1">
                          {item.title}
                        </span>
                        <span className="text-amber-500 font-black ml-3 sm:ml-4 select-none">
                          ✦
                        </span>
                      </Link>
                    ))}
