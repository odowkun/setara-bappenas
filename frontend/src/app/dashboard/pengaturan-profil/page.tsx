...
export default function PengaturanProfilPage() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    const saved = await updateProfile({ name, email, jabatan, nip });
    if (saved) {
      toast.success("Informasi profil Anda berhasil diperbarui!");
    } else {
      toast.error("Profil gagal diperbarui. Periksa data atau sesi login.");
    }
  ...
  const handleSavePassword = async (e: React.FormEvent) => {
    ...
    if (
      ...
      toast.error("Kata sandi minimal 12 karakter serta memiliki huruf besar, huruf kecil, dan angka.");
      return;
    }
    ...
    if (saved) {
      ...
      toast.error("Kata sandi gagal diperbarui. Periksa kata sandi lama dan ketentuan keamanan.");
    }
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              ...
              {user?.role === "admin_bidang" && (
                <div>
                  ...
                  <div className="relative rounded-2xl bg-slate-100 border border-slate-200 px-4 py-3">
                    <Building2 className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                    <p className="pl-6 text-xs font-black uppercase text-slate-700">
                      {user.bidang || "Belum ditetapkan"}
                    </p>
                  </div>
                  <p className="mt-1 text-[10px] font-medium text-slate-500">
                    Perubahan bidang hanya dapat dilakukan Super Admin karena menentukan cakupan data.
                  </p>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Minimal 12 karakter, huruf besar/kecil, angka"
                      className="w-full pl-10 pr-11 py-2.5 rounded-2xl bg-slate-50 border border-slate-200 focus:border-blue-700 focus:bg-white text-xs font-bold text-slate-900 placeholder-slate-400 focus:outline-none transition shadow-2xs"
                    />
                    <button
                      type="button"
