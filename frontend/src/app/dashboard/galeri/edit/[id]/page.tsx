...
export default function EditGaleriPage() {
  ...
  useEffect(() => {
    if (itemId) {
      setIsAlbumMode(true);
      setAlbumName("Dokumentasi Musrenbang RKPD 2026 Kabupaten Halmahera Utara");
      setEventDate("2026-07-15");
      setDescription("Foto dan video suasana pelaksanaan Musrenbang RKPD TA 2026 bertempat di Aula Bappeda Halut.");
      const loaded: MediaUploadItem[] = [
        {
          id: "m-1",
          type: "image",
          url: "https://images.unsplash.com/photo-1541872703-74c5e44368f9?auto=format&fit=crop&w=800&q=80",
          title: "Pembukaan Musrenbang RKPD oleh Sekretaris Daerah",
          fileSize: "2.4 MB",
        },
        {
          id: "m-2",
          ...
          title: "Sesi Diskusi Kelompok Bidang Fisik & Prasarana",
          fileSize: "1.8 MB",
        },
        {
          id: "m-3",
          ...
          title: "Video Highlight Pembahasan Musrenbang 2026",
          fileSize: "14.5 MB",
        },
      ];
  ...
  const handleFileUpload = (files: FileList) => {
    ...
    Array.from(files).forEach((file, index) => {
      ...
      const newId = `media-${Date.now()}-${index}`;
      newItems.push({
        id: newId,
    ...
    if (isAlbumMode) {
      ...
      if (newItems.length > 0) setCoverMediaId(newItems[0].id);
    }
  };
    ...
    <div className="space-y-4 w-full max-w-[1400px] mx-auto font-sans">
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-0.5">
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              <FolderEdit className="w-6 h-6 text-blue-600 shrink-0" />
              <span>Edit Galeri Dokumentasi</span>
            </h1>
            <p className="text-xs text-slate-500 font-medium leading-relaxed">
              Perbarui nama album, tanggal kegiatan, foto sampul depan, serta kelola foto & video di dalamnya.
            </p>
          </div>
        </div>
        ...
        {isSaved && (
          <div className="px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-black flex items-center gap-2 animate-in fade-in shrink-0">
            ...
            <span>Perubahan Galeri Berhasil Disimpan!</span>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium">
                {isAlbumMode
                  ...
                  : "Mode Single: Centang di sini jika ingin menjadikan pengelompokan album."}
              </p>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-xs space-y-4">
          ...
          <label className="p-6 rounded-3xl border-2 border-dashed border-slate-300 hover:border-blue-500 bg-slate-50 hover:bg-blue-50/50 cursor-pointer text-center block transition group">
            <UploadCloud className="w-8 h-8 text-blue-600 mx-auto group-hover:scale-110 transition" />
            <div className="mt-1.5">
              <p className="font-black text-slate-900 text-xs">
                + Tambah Berkas Foto / Video ke Dalam Album Ini
              </p>
              <p className="text-slate-500 text-[11px] font-medium">
                Pilih satu atau banyak berkas sekaligus (JPG, PNG, WEBP, MP4, WEBM)
              </p>
            </div>
            <input
              type="file"
