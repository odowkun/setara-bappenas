...
export default function EditStrukturPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Form State
  const [currentPejabat, setCurrentPejabat] = useState<OfficialItem | null>(null);
  ...
  // Fetch all pejabat list and target single pejabat by ID
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (allRes.ok) {
          ...
          setOfficials(allJson.data.flat);
        }

        if (targetRes.ok) {
          const targetJson = await targetRes.json();
          const p = targetJson.data;
          setCurrentPejabat(p);
          setPosition(p.position);
          setParentId(p.parent_id || "");
          setName(p.name === "(Belum Ditentukan)" ? "" : p.name);
          setNip(p.nip || "");
          setAvatar(p.avatar || "");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Data pejabat "${position}" berhasil diperbarui!`);
        router.push("/dashboard/profil/struktur");
      } else {
        toast.error("Gagal memperbarui data pejabat di database!");
      }
    } catch (err) {
      console.error("Gagal memperbarui data pejabat:", err);
      toast.error("Terjadi kesalahan koneksi saat memperbarui data pejabat!");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-8 max-w-4xl mx-auto font-sans pb-16">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-slate-400 font-bold">
          Single Page Form — Edit Struktur #{id}
        </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
          <div className="border-b border-slate-100 pb-3">
            <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
              ...
              <span>1. Edit Posisi Jabatan & Atasan Hirarki</span>
            </h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Ubah posisi jabatan dan kedudukan atasannya dalam struktur organisasi.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="space-y-1.5 relative">
              ...
              <button
                ...
                className="w-full px-4 py-3.5 rounded-2xl bg-slate-50 border border-slate-200 hover:border-emerald-500 text-left text-xs font-black text-slate-900 flex items-center justify-between transition"
              >
                ...
                <div className="absolute left-0 right-0 top-full mt-2 z-50 bg-white border border-slate-200 rounded-2xl shadow-xl max-h-60 overflow-y-auto p-2 space-y-1 font-sans animate-in fade-in">
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-bold flex items-center justify-between transition ${
                      !parentId ? "bg-emerald-50 text-emerald-900" : "hover:bg-slate-100 text-slate-700"
                    }`}
                  >
                    ...
                    {!parentId && <Check className="w-4 h-4 text-emerald-600" />}
                  </button>

                  <div className="border-t border-slate-100 my-1" />

                  {hierarchicalTreeList
                    .filter(({ node_id }) => !currentPejabat || node_id !== currentPejabat.node_id)
                    .map(({ node_id, position: pos, depth }) => {
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
          <div className="border-b border-slate-100 pb-3">
            <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
              ...
              <span>2. Data Lengkap Pejabat & Foto Resmi</span>
            </h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Isi atau perbarui nama lengkap, NIP, serta foto profil resmi pejabat.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 font-mono font-bold text-xs text-slate-800 focus:border-emerald-600 focus:bg-white focus:outline-none transition"
              />
            </div>
            ...
            <div className="sm:col-span-2 space-y-2">
              <label className="block text-xs font-black text-slate-800 uppercase">
                Foto / Avatar Resmi Pejabat (Otamatis Konversi WebP & HD):
              </label>
              ...
              {avatar && (
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-emerald-50 border border-emerald-200 mt-2">
                  ...
                  <div className="text-xs space-y-0.5">
                    <p className="font-extrabold text-emerald-950">Foto Profil Terpasang</p>
                    <p className="text-emerald-700 font-mono text-[10px] truncate max-w-md">{avatar}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-lg shadow-emerald-600/25 flex items-center justify-center gap-2 transition disabled:opacity-50 cursor-pointer"
          >
            <Save className="w-4 h-4" />
            <span>{saving ? "Perbarui Data..." : "Perbarui Data Posisi & Pejabat"}</span>
          </button>
        </div>
      </form>
    </div>
