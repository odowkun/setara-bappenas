...
- **Konfigurasi Standalone Next.js Bebas Webpack Alias Rapuh**: `next.config.ts` menggunakan konfigurasi murni `output: "standalone"` tanpa alias custom `@swc/helpers` yang rawan memicu `MODULE_NOT_FOUND` saat evaluasi konfigurasi oleh `next/dist/server/config.js` di Windows runner. Dependensi frontend selalu diinstal penuh dengan `npm install --legacy-peer-deps --no-audit`.
