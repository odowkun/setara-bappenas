...
- **Akselerasi Build & Nonaktifkan Telemetri**: Build Next.js di runner Windows menggunakan `NEXT_TELEMETRY_DISABLED=1`, `CI=1`, dan `NODE_OPTIONS="--max-old-space-size=4096"`. Cache `.next\cache` dipertahankan untuk build inkremental yang cepat, serta `ignoreBuildErrors: true` & `ignoreDuringBuilds: true` diaktifkan karena lint dan typecheck telah diverifikasi pada lingkungan staging lokal.
- **Konfigurasi Standalone Next.js Native ESM (`next.config.mjs`)**: Menggunakan format native ESM murni `next.config.mjs` (bukan `.ts`) agar dievaluasi langsung oleh Node.js via `import()` tanpa melalui loader `next-config-ts/transpile-config.js` atau dependensi compile-time pada runtime server. Dependensi frontend selalu diinstal penuh dengan `npm install --include=dev --legacy-peer-deps --no-audit` sehingga tools build (`tailwindcss`, `postcss`, dll.) selalu lengkap tersedia.
